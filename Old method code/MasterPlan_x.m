...
%%
%the following plots the original data (now named "transposedata") for each cell k with its
%corresponding criterion line (mean baseline + mstandard deviations)
fig = gcf;
ax = axes('Parent', fig);
CRITERION = [];
...
%% the following calculates the area under the curve (AUC) between each stimulus onset to its
%offset
...
%% This section finds each stimulus and calculates a new baseline for the response to
%that stimulus. The new baseline is calculated as the mean of the 10 scans
...
begin=StimulusArray(j,2);
NewBase(i,1)=i;
NewBase(i,2)=mean(transposedata((begin-11):(begin-1),i));
end
end
%% the following finds the peak response after each stimulus
...
T = table(Cell,AUC(:,2:s_number+1),MAX(:,2:s_number+1),dF_F,'VariableNames',{'Cell' 'AUC' 'MAX'
'dF'});
