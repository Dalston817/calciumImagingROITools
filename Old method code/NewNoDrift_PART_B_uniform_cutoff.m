...
%The variable "q" is entered by user in NewNoDrive_PART_A.
%Section 1 tests whether responses are <m> times the mean + standard deviation of the first "q" points( i.e "baseline").  
...
while k <= max_k
    ...
    sd_raw=std(data(o,t0-10:t0)); %the variable "sdraw" is the stand dev of the 10 points preceding each stimulus 
    Y_raw(o,StimulusArray(j,2):StimulusArray(j,3))=mean((data(o,t0-10:t0)))+m*sd_raw;  %this command
    if MAX(o,j+1)<mean((data(o,t0-10:t0)))+m*sd_raw   %this tests whether the signal ("data") is < [mean of 10 points before stim + "m" stand deviations]
        cutoff_raw(o,j+1)=0;
    else
    ...
    sd_corrected=std(result(o,1:q)); %the variable "sd" is the stand dev of the first "q" points of the scan 
    Y_corrected(o,StimulusArray(j,2):StimulusArray(j,3))=mean((result(o,1:q)))+m*sd_corrected;  %this command
    if MAXnodrift(o,j+1)<mean((result(o,1:q)))+m*sd_corrected   %this tests whether the signal ("result") is < [mean of first q points of scan + "m" stand deviations]
