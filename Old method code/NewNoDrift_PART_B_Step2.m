...
while k <= max_k
    ...
    if MAX(o,j+1)<mean((data(o,1:q)))+m*sd_raw   %this tests whether the signal ("data") is < [mean of 10 points before stim + "m" stand deviations]
        ...
        cutoff_raw(o,j+1)=MAX(o,j+1)-(mean((data(o,1:q)))+m*sd_raw);
    end
    subplot(2,1,2) 
    ...
    if MAXnodrift(o,j+1)<mean((result(o,1:q)))+m*sd_corrected   %this tests whether the signal ("result") is < [mean of baseline + "m" stand deviations]
        ...
        cutoff_corrected(o,j+1)=MAXnodrift(o,j+1)-(mean((result(o,1:q)))+m*sd_corrected);
    end
    hold off;
    subplot(2,1,1)
...
xlswrite(filename,{file},'5 MAX-cutoff_raw data','A1'); %this enters the location/name of the data file in the first cell of a worksheet named "3 MAX for raw data"
T5 = table(Cell,emptyCol,cutoff_raw(:,2:s_number+1),'VariableNames',{'Cell' 'blnk' 'MAX_above_cutoff'});
writetable(T5,filename,'Sheet',"5 MAX-cutoff_raw data",'Range','b4');
xlswrite(filename,{' threshold criterion='},'5 MAX-cutoff_raw data','A2');
xlswrite(filename,m,'5 MAX-cutoff_raw data','D2');
xlswrite(filename,{'x stand deviation of baseline'},'5 MAX-cutoff_raw data','E2');
%%
xlswrite(filename,{file},'6 MAX-cutoff_drift-corrected','A1'); %this enters the location/name of the data file in the first cell of a worksheet named "4 MAX for drift-corrected"
T6 = table(Cell,emptyCol,cutoff_corrected(:,2:s_number+1),'VariableNames',{'Cell' 'blnk' 'MAX_above_cutoff'});
writetable(T6,filename,'Sheet',"6 MAX-cutoff_drift-corrected",'Range','b4');
xlswrite(filename,{' threshold criterion='},'6 MAX-cutoff_drift-corrected','A2');
xlswrite(filename,m,'6 MAX-cutoff_drift-corrected','D2');
xlswrite(filename,{'x stand deviation of baseline'},'6 MAX-cutoff_drift-corrected','E2');
