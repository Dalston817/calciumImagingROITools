%%
%This script calculates the stand dev ("noise") in a signal for 10 consecutive point, similar to a "moving average with window=10" but instead a "moving st dev with window'10"
%The product is an array named "stdev_result"
...
for j=5:(size(result,2)-5)   %this reads the array "result" beginning with the 4th point up to 5 points before the end
    stdev_result(i,j)=std(result(i,(j-4):(j+5)));  %this calculates the stand dev of 10 points around "j" and enters it into the array named "stdev_result"
end
end
%%
%%
...
k = 1;
while k <= max_k
plot(result(k,:),'Color',[0.929,0.855,0.22]);hold on;plot(stdev_result(k,:),'k');
